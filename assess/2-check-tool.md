...
A tool must be added to the register before it is used.
...
Facts go stale. Suppliers change terms, models are updated, and tiers are renamed. Record who gathered the facts, who signed off the decisions, and when the profile is next due for review. Treat a profile that has not been checked recently as a starting point rather than a current record.
